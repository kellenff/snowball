...
## Hard constraints (verified against source + live round-trip)

1. **A canonical section model, but storage is verbatim.** codebase-memory recognizes 6 exact, case-sensitive section names — `PURPOSE`, `STACK`, `ARCHITECTURE`, `PATTERNS`, `TRADEOFFS`, `PHILOSOPHY` (`is_canonical_section`, `src/store/store.c:4450`). That canonical filtering (via `cbm_adr_parse_sections`) governs the **section-targeted update path** (`cbm_store_adr_update_sections`) and **content validation** (`cbm_adr_validate_content`) — **not** full-document storage. A live round-trip (2026-05-31) confirmed that `manage_adr(mode='update')` stores the full content **verbatim** and `mode='get'` returns it byte-for-byte: even a non-canonical `## DECISIONS` header survives intact. So a dedicated decisions *section* is not physically dropped — but it is invisible to codebase-memory's structured section model. We fold decision content into `TRADEOFFS`/`PHILOSOPHY` to stay **inside** that structured model (the sections codebase-memory's own tooling and validation understand), not because a custom section would be erased. `mode='sections'` (read) is a naive `##` header scan and must not be relied on for canonical filtering.
2. **Whole-document writes.** `manage_adr(mode='update', content=...)` replaces the entire stored ADR with the supplied string. There is no full per-section write API exposed here; any update carries the full intended document. The skill's load-bearing dependency is the verbatim round-trip of this full document (it carries the digest marker).
