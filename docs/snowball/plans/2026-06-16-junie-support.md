...
- [x] **Step 1: Create the directory and `extension.json`**
...
- [x] **Step 2: Symlink the skills directory**
...
- [x] **Step 3: Copy the `using-snowball` text into `.junie/AGENTS.md`**
...
- [x] **Step 1: Add `approval-phrases` to the build entries**
...
- [x] **Step 3: Verify the bundle exports `matchesApproval`**
...
- [x] **Step 3: Write `src/session-id.ts`**
...
- [x] **Step 4: Write `src/schemas.ts` (zod schemas for the three tools)**
...
- [x] **Step 5: Write `src/server.ts` (minimal — no tools yet)**
...
- [ ] **Step 6: Write a smoke test that the server starts (no tools registered)** — *Done with drift: implemented as `tests/snowball-capture/smoke.test.ts` (bun:test) instead of `extensions/snowball/snowball-capture/scripts/smoke.sh`. Same coverage (spawn the bundled server, send MCP `initialize`, assert response mentions `snowball-capture`).*
...
- [x] **Step 7: Install deps, build, run the smoke test**
...
- [x] **Step 2: Run the tests to confirm they pass (the schema is the only implementation so far)**
...
- [x] **Step 4: Run the integration test — it fails (no implementation yet)**
...
- [x] **Step 5: Implement `src/tools/madr-capture.ts`**
...
- [x] **Step 6: Run the integration tests — they pass**
...
- [x] **Step 2: Run unit tests (the schema is in place from Task 3)**
...
- [x] **Step 3: Append integration tests for `approval_phrase_record`**
...
- [x] **Step 4: Run the new integration tests — they fail**
...
- [x] **Step 5: Implement `src/tools/approval-phrase-record.ts`**
...
- [x] **Step 6: Run all integration tests**
...
- [x] **Step 3: Append integration tests for `observation_log`**
...
- [x] **Step 4: Run — fails (no implementation)**
...
- [x] **Step 5: Implement `src/tools/observation-log.ts`**
...
- [x] **Step 2: Rebuild and rerun the smoke test**
...
- [ ] **Step 1: Write a one-shot script that generates the fixtures** — *Done with drift: fixtures were hand-curated and checked in at `tests/snowball-capture/integration/fixtures/` (canonical-madr.md, canonical-observation.jsonl). No generator script exists; the contract test compares live MCP output to the hand-curated fixture modulo timestamp/event_id/session_id.*
...
- [ ] **Step 2: Run the script once to generate the fixtures** — *Not applicable; see Step 1 drift note. Fixtures are checked-in artifacts, not generated.*
...
- [ ] **Step 3: Delete the generator script** — *Not applicable; see Step 1 drift note. No generator script was ever written.*

```bash
rm tests/snowball-capture/integration/_gen-fixtures.ts
```

- [x] **Step 4: Write the contract test**
...
- [x] **Step 1: Append the capture rules section to AGENTS.md**
...
- [x] **Step 2: Write the drift-guard test**
...
- [x] **Step 1: Read the existing tail of the config to anchor the insertion point**

```bash
tail -25 .pre-commit-config.yaml
```

- [x] **Step 2: Append the two new hooks**
...
- [x] **Step 4: Run the new build script manually to verify**
...
- [x] **Step 5: Run the new test hook manually**
...
- [x] **Step 6: Verify pre-commit config is valid YAML**
...
- [x] **Step 1: Add the Junie row to the per-harness adapters table**
...
- [x] **Step 2: Add a Junie install snippet in the Setup section**
...
- [x] **Step 3: Add a `v6.3.0` row to the changelog**
...
- [x] **Step 4: Add the `v6.3.0` section to RELEASE-NOTES**
...
- [x] **Step 5: Verify both files parse cleanly**
...
- [ ] **Step 1: Walk through the spec's manual verification section** — *Deferred to operator. Requires a live JetBrains IDE + Junie install. All prerequisite work (extension scaffold, AGENTS.md, MCP server, capture tools) is committed; the spec's "Manual verification" section in `docs/snowball/specs/2026-06-16-junie-support-design.md` is the operator's checklist.*
...
- [ ] **Step 2: Write the verification report** — *Deferred to operator (see Step 1).*
...
- [ ] **Step 3: Commit (only if the report has findings worth recording)** — *Deferred to operator (see Step 1).*
...
This plan was implemented and committed in the following sequence: Tasks 1–12 landed across commits `a23cb4d` → `4f6cda0` (see `git log --oneline -- extensions/snowball/ tests/snowball-capture/`). The 70 plan checkboxes above have been updated to reflect actual on-disk and git state.

**Drift from plan as written (work is done; just implemented differently):**

- **Task 3, Step 6** — smoke test was implemented as `tests/snowball-capture/smoke.test.ts` (a `bun:test` file) rather than `extensions/snowball/snowball-capture/scripts/smoke.sh`. Functionally equivalent: spawn the bundled `dist/server.cjs`, send an MCP `initialize` request, assert the response contains `snowball-capture`. Lives in the test suite where it can be wired into CI/pre-commit (which it is, via `bun-test-snowball-capture`).
- **Task 8, Steps 1–3** — the contract-test fixtures at `tests/snowball-capture/integration/fixtures/` (canonical-madr.md, canonical-observation.jsonl) were hand-curated and checked in directly, not generated by a one-shot script that was then deleted. The contract test (`tests/snowball-capture/integration/contract.test.ts`) compares live MCP output against the hand-curated fixture, normalizing only `timestamp`, `event_id`, and `session_id`. Same coverage, no throwaway script.

**Deferred to operator (genuinely not done; requires a live environment):**

- **Task 13, all steps** — manual end-to-end verification requires a JetBrains IDE with Junie installed. The spec's "Manual verification" section is the operator's checklist. All prerequisite work (extension, AGENTS.md, MCP server, three capture tools, contract test) is committed and CI-runnable; what remains is to install the extension in a real IDE and observe captures.
