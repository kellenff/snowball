...
import {
  ...
  normalizeVtcodeAnswers,
} from "../../skills/decision-logging/src/vtcode-post-tool-use-bridge";
import {
  normalizeAnswers,
  normalizeQuestions,
...
// These cover the pure handlers extracted so the OpenCode plugin can
    ...
    questions: [
      {
        ...
        header: "Storage",
        options: [{ label: "Two-tier", description: "MADR + JSONL" }],
      },
    ],
  };
  const questions = normalizeQuestions(toolInput);
  const answers = normalizeVtcodeAnswers(questions, {
    answers: { storage: { selected: ["Two-tier"], other: null } },
  });
...
test("normalizeVtcodeAnswers ignores questions with no answer", () => {
  const toolInput = {
    questions: [
  ...
  const questions = normalizeQuestions(toolInput);
  const answers = normalizeVtcodeAnswers(questions, {
    answers: { a: { selected: ["x"] } },
  });
  expect(answers).toEqual({ "Q1?": "x" });
});

test("normalizeVtcodeAnswers handles missing or malformed answers gracefully", () => {
  const toolInput = {
    questions: [{ id: "a", question: "Q1?", header: "A" }],
  };
  const questions = normalizeQuestions(toolInput);
  expect(normalizeVtcodeAnswers(questions, undefined)).toEqual({});
  expect(normalizeVtcodeAnswers(questions, {})).toEqual({});
  expect(normalizeVtcodeAnswers(questions, { answers: null })).toEqual({});
  expect(normalizeVtcodeAnswers(questions, { answers: { a: { selected: [] } } })).toEqual({});
});
      ...
      questions: [
        {
          ...
          header: "Approach",
          options: [
            { label: "Lazy", description: "ponytail" },
            { label: "Eager", description: "all upfront" },
          ],
        },
        {
          id: "tests",
