    ...
    candidates: [
      {
        ...
        triage_score: 30,
        sample_windows: [
          { sessionId: "s1", startedAt: null, messageSpan: [1, 1], marginalTokens: 30 },
        ],
  ...
  test("writes candidates.json and windows.jsonl under the metrics dir", () => {
    ...
    try {
      ...
      const candidates = JSON.parse(
        fs.readFileSync(path.join(dir, ".snowball/metrics/candidates.json"), "utf8"),
      );
      expect(candidates.candidates[0].skill_name).toBe("a");
      const windows = fs
        .readFileSync(path.join(dir, ".snowball/metrics/windows.jsonl"), "utf8")
        .trim()
        .split("\n");
