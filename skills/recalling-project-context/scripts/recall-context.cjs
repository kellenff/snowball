...
var exports_recall_context = {};
__export(exports_recall_context, {
  renderExcerptForHook: () => renderExcerptForHook,
  prepare: () => prepare
});
...
function parseMadr(filename, raw) {
  ...
  if (!m)
    return { warning: `${filename}: no YAML frontmatter` };
  let fm;
  try {
    fm = load(m[1]);
  } catch (err) {
    return { warning: `${filename}: invalid YAML (${err.message})` };
  }
  ...
  if (!status)
    return { warning: `${filename}: missing status` };
  const snowball = obj.snowball && typeof obj.snowball === "object" ? obj.snowball : {};
  const sourceEventId = typeof snowball.source_event_id === "string" ? snowball.source_event_id : filename;
  return {
    filename,
...
var OBS_REQUIRED = ["session_id", "timestamp", "type", "confidence", "content"];
...
// skills/syncing-decisions-to-memory/src/filter.ts
var KEEP_MADR_STATUSES = new Set(["accepted", "proposed"]);
var KEEP_OBS_TYPES = new Set(["constraint", "implementation-choice"]);
function filterRecords(input) {
  return {
    madrs: input.madrs.filter((m) => KEEP_MADR_STATUSES.has(m.status)),
    observations: input.observations.filter((o) => o.confidence === "high" || KEEP_OBS_TYPES.has(o.type))
  };
}
...
function madrTags(raw) {
  ...
  try {
    const fm = load(m[1]);
    const snowball = fm?.snowball && typeof fm.snowball === "object" ? fm.snowball : {};
...
function madrMatchesScope(record, raw, scope) {
  const needle = normalizeScope(scope);
  if (!needle)
    return true;
  const haystacks = [record.filename, record.title, record.body, ...madrTags(raw)].map((s) => s.toLowerCase());
  return haystacks.some((h) => h.includes(needle));
}
function madrExcerpt(body, cap) {
  const trimmed = body.replace(/\s+/g, " ").trim();
  if (trimmed.length <= cap)
    return trimmed;
  return `${trimmed.slice(0, cap).trimEnd()}…`;
}
function recallMadrs(input) {
  ...
  const madrs = withMeta.slice(0, maxMadrs).map(({ record, mtimeMs }) => ({
...
function prepare(input) {
  ...
  const base = {
    scope,
    madrs: madrResult.madrs.map(({ filename, title, status, excerpt: excerpt2 }) => ({
      filename,
      title,
      status,
      excerpt: excerpt2
...
function renderExcerptForHook(input) {
...
if (require.main == module) {
  ...
  process.stdin.on("end", () => {
    try {
      const input = raw.trim() ? JSON.parse(raw) : { gitRoot: process.cwd() };
      if (sub === "prepare") {
        process.stdout.write(JSON.stringify(prepare(input)));
      } else if (sub === "excerpt") {
        process.stdout.write(renderExcerptForHook(input));
      } else {
        process.stderr.write(`unknown subcommand: ${String(sub)} (expected 'prepare' or 'excerpt')
`);
        process.exit(2);
      }
    } catch (err) {
      process.stderr.write(`recall-context error: ${err.message}
`);
      process.exit(1);
    }
  });
