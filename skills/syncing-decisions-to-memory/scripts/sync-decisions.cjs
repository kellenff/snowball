...
var OWNED_SECTIONS = ["TRADEOFFS", "PHILOSOPHY"];
...
function renderAdr(input) {
  const sections = { ...input.preserved };
...
`);
      ...
      process.stderr.write(`sync-decisions error: ${err.message}
`);
      process.exit(1);
    }
  });
